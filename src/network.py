...
class RNNModel(nn.Module):
    ...
    def __init__(
        ...
        if fc_nonlinearity == "tanh":
            ...
            raise ValueError(
                f"Unsupported non-recurrent nonlinearity: {fc_nonlinearity}"
            )

        fc_layers = []

        fc_layers.append(
            nn.Linear(rec_hidden_size + self.input_size, fc_hidden_sizes[0])
        )
    ...
    def forward(self, x, hn) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(x.shape) == 1:
            ...
            if self.rnn_type == "LSTM":
                hn = (
                    hn[0].transpose(0, 1).contiguous(),
                    hn[1].transpose(0, 1).contiguous(),
                )
            else:
    ...
    def init_hidden(self, batch_size) -> torch.Tensor:
        ...
        if isinstance(self.rnn, nn.LSTM):
            first = torch.squeeze(
                torch.zeros(self.rec_num_layers, batch_size, self.rec_hidden_size)
            )
            second = torch.squeeze(
                torch.zeros(self.rec_num_layers, batch_size, self.rec_hidden_size)
            )
...
class ActorCategorical(nn.Module):
    ...
    def forward(self, states, hn) -> Tuple[Categorical, torch.Tensor, torch.Tensor]:
        ...
        pi = TransformedDistribution(
            pi, AffineTransform(loc=-(self.actor_net.output_size // 2), scale=1.0)
        )
...
class ActorCriticAgent(object):
    ...
    @torch.no_grad()
    def __call__(
        self, state: torch.Tensor, device: str, hn_actor, hn_critic
    ) -> Tuple[
        Distribution,
        torch.Tensor,
        torch.Tensor,
